...
def post(post_id):
    ...
    if post is None:
      ...
      app.logger.debug('Article titled -> {title} is retreived'.format(title=post['title']))
...
def create():
    if request.method == 'POST':
        ...
        if not title:
            ...
            connection.commit()
            connection.close()
            app.logger.debug('Article titled -> {title} is created'.format(title=title))
...
if __name__ == "__main__":
   app.logger.debug('Flask server starting')
